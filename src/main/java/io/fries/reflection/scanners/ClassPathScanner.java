...
public final class ClassPathScanner implements Scanner {
	 ...
	 */
	private ClassPathScanner(final ClassLoader classLoader, final String packagePrefix) {
	 ...
	 *
	 * @see ClassPathScanner#ClassPathScanner(ClassLoader, String)
	 */
	public static ClassPathScanner of(final ClassLoader classLoader, final String packagePrefix) {
		return new ClassPathScanner(classLoader, packagePrefix);
	}
	...
	private void scanClassPath() {
		...
		// final File file = new File("/Users/yumeijie/idea-workspace/reflection/build/classes/java/main/io/fries/reflection");
		// entries.put(file, Thread.currentThread().getContextClassLoader());
		// getClassPathEntries(classLoader).forEach(this::scan);
		// entries.forEach(this::scan);
		final String start = System.getProperty("java.class.path").split(":")[0] + "/" + this.packagePrefix;
		...
		scan(f, Thread.currentThread().getContextClassLoader());
	}
	...
	private void scan(final File file, final ClassLoader classLoader) {
		...
		try {
			if(!file.exists())
			  ...
			  if (file.isDirectory()) {
		        System.out.println("File is a directory " + this.packagePrefix);
				scanDirectory(file, classLoader, this.packagePrefix + "/");
			  } else {
	...
	private void scanDirectory(final File dir, final ClassLoader classLoader, final String packagePrefix) {
		...
		for(File file : files) {
            ...
            System.out.println(resourceName  + " filter is empty: " + filters.isEmpty());
			if(file.isDirectory())
	...
	private boolean addResource(final ClassLoader classLoader, final String resourceName) {
		System.out.println("add resource is called " + resourceName);
	...
	public static void main(String args[]) {
		...
		Class<?> entryPoint = ClassPathScanner.class;
		final String rootPackage = entryPoint.getPackage().getName();
		URL main = entryPoint.getResource("ClassPathScanner.class");
		...
		final Scanner scanner = ClassPathScanner
		 ...
		 Reflection ref =  Reflection.of(scanner);
		 //System.out.println("Before get annotations" + ref.getResources());
		Set<ClassMetadata> classes = ref.getClasses();
		 System.out.println("class" +classes);
		Set<Class<?>> types = ref.getTypes();
		 System.out.println(types);
        // System.out.println( "types" + ref.getAnnotatedTypes(Register.class));
		// System.out.println(ClassPathScanner.class.getAnnotation(Identified.class));
	}*/
}
